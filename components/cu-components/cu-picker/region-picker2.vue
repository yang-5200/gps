<template>
	<view class="cu-picker-view">
		<picker-view class="d-picker-view" :indicator-style="itemHeight" :value="pickVal" @change="handlerChange">
			<picker-view-column>
				<view class="cu-picker-item" v-for="(item,index) in range.provinces" :key="index">{{item.label}}</view>
			</picker-view-column>
			<picker-view-column>
				<view class="cu-picker-item" v-for="(item,index) in range.citys" :key="index">{{item.label}}</view>
			</picker-view-column>
			<picker-view-column v-if="!hideArea">
				<view class="cu-picker-item" v-for="(item,index) in range.areas" :key="index">{{item.label}}</view>
			</picker-view-column>
		</picker-view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods:{
			/**
			 * @description 获取已开通城市列表
			 */
			getCitylist(){
				return new Promise((resolve, reject) =>{
					this.$api.get(uni.apiUrls.getCitylist).then(res => {
						// //console.log('获取已开通城市列表', res);
						if(res.code){
							let list = this.handleCityData(res.data);
							// //console.log(list)
							resolve(list)
						}else{
							resolve([])
			...
			handleCityData(list){
				return list.map((item, index) => {
					...
					if(item.childlist.length){
						item.children = this.handleCityData(item.childlist);
					}else{
						item.children = [];
					}
					return item;
			...
			getData(){
				...
				// //console.log('areas',citys[0],areas,cityIndex);
			...
			async initData(){
				this.areaData = await this.getCitylist();
