<template>
	<view class="sku" v-if="showSkuPopup" :class="skuPopupStyleShow ? 'show' : 'hide'"
		...
		<view class="shopSpecsPopup" :class="shopSpecsPopupStyleShow ? 'show' : 'hide'" @click.stop>
			...
			<view class="content">
				<view class="info flex f-y-c">
					...
					<view class="right t-w" :class="{useImgSku: isUseImgSku}">
						<!-- :style="{color: themeRGB}" -->
						<view>{{ goodsName }}</view>
						<view class="price f-y-e">
							<text class="uity">￥</text>
							<template v-if="isSku">
								<template v-if="selectSku.id">
									{{ selectSku.price.toFixed(2).split('.')[0] }}.<text class="text-sm">{{ selectSku.price.toFixed(2).split('.')[1] }}</text>
								</template>
								<template v-else>
									<template v-for="(price, index) in showAreaPrice">
										{{ price.toFixed(2).split('.')[0] }}.<text class="text-sm">{{ price.toFixed(2).split('.')[1] }}</text>
							...
							<template v-else>
								<template>
									{{ Number(goodsPrice).toFixed(2).split('.')[0] }}.<text class="text-sm">{{ Number(goodsPrice).toFixed(2).split('.')[1] }}</text>
								</template>
							</template>
				...
				<view class="number flex f-y-c f-x-b">
					...
					<numBox :width="200" integer v-model="num"
			...
			<view class="btnBox">
				<!-- :style="{background: themeRGB}" -->
				<view class="shoppingCart" :class="{
						disabled: isSku && selectSku.id && selectSku.stock * 1 < 1
					}"  @click="confirm(2)">
					加入购物车
				</view>
...
<script>
	 ...
	 * @property {Boolean} isSku 是否多规格。
	 * @property {String} goodsPrice 单规格商品价格。
	 * @property {String} singleStock 单规格库存。
	...
	export default {
		...
		computed: {
			numMinValue() {
				if (this.isSku) {
					return this.selectSku.id ? (this.selectSku.stock === 0 ? 0 : 1) : 1;
				} else {
					// return this.singleStock === 0 ? 0 : 1;
					return 1
				}
			},
			numMaxValue() {
				if (this.isSku) {
					return this.selectSku.id ? this.selectSku.stock : this.showAreaStock[1];
				} else {
	...
	.sku {
		...
		.shopSpecsPopup {
			...
			.btnBox {
				...
				.shoppingCart{
					color:#44D4A8;
					background: rgba(68,212,168,0.2);
